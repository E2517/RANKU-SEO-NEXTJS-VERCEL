    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (infoEl) {
                    infoEl.innerHTML = `
                        <strong>Búsqueda ${data.campaign.status === 'completed' ? 'completada' : 'en progreso'}:</strong> "${data.campaign.keyword}" en "${data.campaign.centerLocation.name}"<br>
                        <strong>Dominio:</strong> ${data.campaign.domain}<br>
                        <strong>Radio:</strong> ${data.campaign.maxRadiusMeters} metros | <strong>Franja:</strong> ${data.campaign.stepMeters} metros
                    `;
                }
    ...
    const renderMap = async (campaign: any, results: any[]) => {
        ...
        leafletMap.current.eachLayer((layer: any) => {
            if (layer instanceof L.Marker) {
                ...
                if (popup) {
                    ...
                    if (!contentStr.includes('Ubicación Central')) {
        ...
        results.forEach((result: any) => {
            ...
            const marker = L.marker([result.searchLocation.lat, result.searchLocation.lng], { icon: iconDiv })
                .addTo(leafletMap.current);
            const label = result.placeName ? `Encontrado: ${result.placeName}` : 'No encontrado';
            const rankingInfo = result.ranking > 0 ? `<b>Ranking:</b> ${result.ranking}` : 'No encontrado';
            marker.bindPopup(`
                <b>Punto de búsqueda</b><br>
                ${label}<br>
                Distancia: ${Math.round(result.distanceFromCenter)} m<br>
                ${rankingInfo}
            `);
        });
    ...
    return (
        <div className={styles.card}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className={styles.formGroup}>
                    <label>Radio Máximo (metros):</label>
                    <input
                        type="number"
                        className={styles.input}
                        value={radius}
                ...
                <button className={styles.button} type="submit" disabled={isProcessing}>
                    {isProcessing ? 'Iniciando...' : 'Iniciar Simulación'}
                </button>
            </form>
            {showResults && (

...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (typeof isActive !== 'boolean') {
            return NextResponse.json({ success: false, message: 'isActive debe ser booleano' }, { status: 400 });
        }

        const days = Number(trialPeriodDays);
        if (isNaN(days) || days < 1 || days > 90) {
            return NextResponse.json({ success: false, message: 'trialPeriodDays debe ser un número entre 1 y 90' }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: false, message: 'Error al guardar configuración' }, { status: 500 });
    }
}
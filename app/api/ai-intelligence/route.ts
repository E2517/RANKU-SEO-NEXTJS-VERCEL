...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (serpData.error || serpData.search_metadata?.status !== 'Success') {
            return NextResponse.json({ success: false, message: 'Error al obtener resultados de IA.' }, { status: 500 });
        }
        ...
        if (finalPosition !== null) {
            ...
            return NextResponse.json({
                success: true,
                message: 'Búsqueda completada y guardada. Puedes ver los resultados en Dominios o Estadísticas.',
                results: {
            ...
            return NextResponse.json({
                success: true,
                message: 'Búsqueda completada. El negocio o dominio no fue mencionado en la respuesta de la IA.',
                results: {
                    keywords: keywords.trim(),
                    domain: normalizedDomain,
                    business: normalizedBusiness,
        ...
        console.error('Error en /api/ai-intelligence:', error.response?.data || error.message);

...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const worksheetData = uniqueRecords.map(r => ({
            'Palabra Clave': r.palabraClave || '',
            'Dominio': r.dominio || '',
            'Posición': r.posicion !== null && r.posicion !== undefined ? r.posicion : '—',
            'Buscador': r.buscador || '',
            'Dispositivo': r.dispositivo || '',
            'Localización': r.location || '',
            'Última Actualización': r.updatedAt ? new Date(r.updatedAt).toLocaleString('es-ES') : ''
        }));
        ...
        headers.set('Content-Disposition', `attachment; filename="informe_seo_${domain || 'todos'}.xlsx"`);

        return new NextResponse(excelBuffer, { headers });
    } catch (err) {
        console.error('Error al generar Excel:', err);
        return NextResponse.json({ success: false, message: 'Error al generar el archivo Excel.' }, { status: 500 });
    }
}